	...
	function outputBitmap(bmp, element){
		...
		if(parentLayer && parentLayer.layerType == "folder" && pattUseLayer.test(parentLayer.name)){
			...
			if(lastFolderName != parentLayer.name) {
				...
				outputDefinitions += "		public var "+lastFolderName+":Sprite = new Sprite();\n";
			}
		} else {
			container = "";
		}
		...
		var name = bitmapName.replace(patt,"");
		outputImageNames += "\""+name+"\"\n";
		outputTmp       = "			img = Assets.getImage('"+name+"');\n"
